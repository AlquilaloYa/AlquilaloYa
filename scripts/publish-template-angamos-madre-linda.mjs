...
const CONTENIDO = fs.readFileSync("apps/web/src/lib/pdf/template-angamos-madre-linda.html", "utf8");
const sql = postgres(process.env.DATABASE_URL, { max: 1 });
try {
  const result = await sql.begin(async (tx) => {
    const rows = await tx`SELECT id FROM templates WHERE clave = ${"ANG_LINDA_MADRE"} LIMIT 1`;
    let templateId;
    if (!rows[0]) {
      const created = await tx`
        INSERT INTO templates (clave, nombre)
        VALUES (${"ANG_LINDA_MADRE"}, ${"Linda_ extensión_ madre_ Angamos"})
        RETURNING id`;
